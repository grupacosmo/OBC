...
{
    Serial.print("\nTime: ");
    if (gps_time.hour < format_helper[0]) { Serial.print('0'); }
    Serial.print(gps_time.hour, DEC);
    Serial.print(':');
    if (gps_time.minute < format_helper[0]) { Serial.print('0'); }
    Serial.print(gps_time.minute, DEC);
    Serial.print(':');
    if (gps_time.seconds < format_helper[0]) { Serial.print('0'); }
    Serial.print(gps_time.seconds, DEC);
    Serial.print('.');
    if (gps_time.milliseconds < format_helper[0]) { Serial.print("00"); }
    else if (
        gps_time.milliseconds > format_helper[1]
        && gps_time.milliseconds < format_helper[2]) {
        Serial.print("0");
    }
...
void print(const GpsPosition& gps_position)
{
    ...
    if (gps_position.fix) {
        ...
        Serial.println(gps_position.speed / velocity_conversion);
